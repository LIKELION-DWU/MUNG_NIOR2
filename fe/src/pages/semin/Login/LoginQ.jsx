...
const LoginQ = () => {
    ...
    if (nameQ.trim() === "" || phoneQ.trim() === "") {
      ...
      try {
        ...
        if (response.status === 200) {
          localStorage.setItem("TokenQ", response.data.Token);
          alert("로그인에 성공했습니다.");

          localStorage.setItem("loggedInUserNameQ", nameQ);
          localStorage.setItem("loggedInUserPhoneQ", phoneQ);
  ...
  return (
    <Container>
      ...
      <Choice>
        <img
          src={`${process.env.PUBLIC_URL}/images_semin/qorange.png`}
          alt="qorange"
          width="230px"
        />
        <img
